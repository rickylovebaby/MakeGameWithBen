...
void Player::init(float speed, glm::vec2 pos, Bengine::InputManager* inputManager, Bengine::Camera2D* camera,std::vector<Bullets>* bullets) {
	_speed = speed;
	_position = pos;
...
void Player::update(const std::vector<std::string>& levelData,
	...
	if (_currentGunIndex != -1) {
		_guns[_currentGunIndex]->update(_inputManager->isKeyPressed(SDL_BUTTON_LEFT), centerPosition,
			direction,*_bullets);
	}
